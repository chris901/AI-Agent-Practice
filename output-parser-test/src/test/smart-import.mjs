...
// 使用 StructuredOutputParser 解析模型输出（避免底层 response_format: json_object 的限制）
...
    await connection.query(`USE hello;`);
    ...
    const prompt = `请从以下文本中提取所有好友信息，文本中可能包含一个或多个人的信息。请将每个人的信息分别提取出来，返回一个 JSON 数组，数组元素为 JSON 对象。
...
4. 返回格式必须是一个 JSON 数组，即使只有一个人也要放在数组中，只输出 JSON 数据，不要输出多余说明文字
